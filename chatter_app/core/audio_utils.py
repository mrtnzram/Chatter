...
class AudioFeatureExtractor:
    ...
    def detect_repeating_mfcc_patterns(
        self, mfcc, threshold=100, min_length=3, min_repeats=2
    ):
        """
    ...
    def refine_regions_with_mfcc_and_energy(self, active_regions, mfcc, rms_energy):
        ...
        min_len = min(
            len(active_regions), len(mfcc_variance), len(rms_energy), mfcc.shape[1]
        )
        ...
        pattern_mask = self.detect_repeating_mfcc_patterns(
            mfcc[:, :min_len], threshold=100, min_length=3, min_repeats=2
        )
    ...
    def get_bouts(self, refined_regions, sr, audio_duration=None):
        ...
        filtered = [
            b for b in merged if (b["offset"] - b["onset"]) >= self.min_bout_length
        ]
        return filtered
    ...
    def compute_all_features(self, row):
        ...
        refined_regions = self.refine_regions_with_mfcc_and_energy(
            active_regions, mfcc, rms_energy
        )
        ...
        if (
            getattr(self, "use_birdnet", False)
            and self.birdnet_model is not None
            and len(bouts) > 0
        ):
            birdnet_flags = self.classify_bouts_with_birdnet(
                audio, sr, bouts, outlier_flags
            )
            for bout, birdnet_flag in zip(bouts, birdnet_flags):
                bout["birdnet_flag"] = (
                    int(birdnet_flag) if birdnet_flag is not None else None
                )
        return pd.Series(
            {
                "audio": audio,
