...
class ChatterController:
    def __init__(
        self, df, extractor, duckdb_path="chatter.duckdb", csv_path="bouts.csv"
    ):
        self.df = df
    ...
    def _apply_params(self, params: dict):
        ...
        self.extractor.active_region_threshold_pct = params.get(
            "active_region_thresh", 0.001
        )
    ...
    def recompute(self, idx: int, params: dict, force: bool = False):
        ...
        if not isinstance(row.get("audio"), np.ndarray):
            ...
            chunk_dur = (
                float(chunk_end - chunk_start) if chunk_end is not None else None
            )
    ...
    # ------------------------------------------------------------------
    # Chunk bout-numbering helper
    # ------------------------------------------------------------------

    def get_bout_label_offset(self, idx: int) -> int:
        """Return the cumulative bout count for all prior chunks of the same recording.

        Used to display continuous bout numbers across chunks in the UI and
        in exported filenames.  Returns 0 for non-chunked recordings and for
        the first chunk.
        """
        row = self.df.iloc[idx]
    ...
    def get_cached_spectrogram(self, wav_location, audio, sr, chunk_start=0.0):
        ...
        cache_key = ChatterStore.make_cache_key(
            ...
            self.extractor.frame_length,
            self.extractor.highpass_cutoff,
            self.extractor.lowpass_cutoff,
        )
        ...
        S = librosa.stft(
            audio,
            n_fft=self.extractor.frame_length,
